...
public class JwtFilter extends GenericFilterBean {
   ...
   private static final String[] whitelist = 
    {"/test","/hello","/signIn","/logout","/css/*"}; //필터 적용을 제외 시킬 리스트를 배열로 만들었다
   ...
   public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
      System.out.println("::::::::  Jwt Filter doFilter() START :::::::::::");
            ...
            if( !useableAccessToken && !useableRefreshToken ){
               ...
               if( !useableAccessToken ){
                  if( !useableRefreshToken ){
                     ...
                     String reissueAccessToken = tokenProvider.createAccessToken(authentication.getName(), requestURI);
                     String reissueRefreshToken = tokenProvider.createRefreshToken(authentication.getName());
                     logger.debug("Security Context에 '{}' 인증 정보를 저장했습니다, uri: {}", authentication.getName(), requestURI);
                     /* Access Token 재발급 */
                     httpServletResponse.setHeader(JwtFilter.AUTHORIZATION_ACCESS_HEADER,"Bearer " + reissueAccessToken);
                     httpServletResponse.setHeader(JwtFilter.AUTHORIZATION_REFRESH_HEADER,"Bearer " + reissueRefreshToken);
                  }
               }
            }
         }
